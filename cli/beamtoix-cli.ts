...
namespace Cli {
  ...
  argOpts['moviePre'] = {
    param: 'array', allowOption: true, desc:
      `arguments to be passed to ffmpeg before the arguments passed by beamtoix`,
  };

  argOpts['moviePost'] = {
    param: 'array', allowOption: true, desc:
      `arguments to be passed to ffmpeg after the arguments passed by beamtoix`,
  };

  argOpts['gifPre'] = {
    param: 'array', allowOption: true, desc:
      `arguments to be passed to gif generator(convert) before the arguments passed by beamtoix`,
  };

  argOpts['gifPost'] = {
    param: 'array', allowOption: true, desc:
      `arguments to be passed to gif generator(convert) after the arguments passed by beamtoix`,
  };
...
The commands are:
    ...
    e.g.
      ...
      beamtoix ${CMD_CREATE} --width 640 --height 480 --fps 25 foo
...
`);
  ...
  function parseArguments(): uint {
    ...
    while (args[argI - 1].search(/beamtoix/) === -1) { argI++; }
...
npm i puppeteer`);
  ...
  function commandCreate(): void {
      ...
      switch (fileBase) {
        ...
        case 'index.html':
          ...
          if (!noPlugins) {
            ...
            let pre = '';
            let post = '';
            text.replace(/^(.*)js\/beamtoix-bundle-en\.min\.js(.*)$/m, (_app, _pre, _post) => {
              pre = _pre;
              post = _post;
              return '';
            });
    ...
    copyTree(LIB_PATH, `${projName}/beamtoix`, undefined,
      (fileBase: string) => {
