...
namespace BeamToIX {
  ...
  export function isCharacter(ch: string | undefined, pos: uint = 0): boolean {
    if (ch === undefined || ch[pos] === undefined) { return false; }
  ...
  function _isId(ch: string | undefined, isFirst: boolean): boolean {
    return ch === undefined ? false :
      (ch === '_' ? true : isFirst ? isCharacter(ch) : isCharacterOrNum(ch));
  }


  function _parseVars(p: ParseParams, varValue: VarType,
    varName: string, expr: string, pos: uint): uint {
  ...
  function _parser(p: ParseParams, checkSign: boolean): TokenClass {
  ...
  function _stateMachine(p: ParseParams): ExprResult {
    ...
    do {
      ...
      const thisTkClass = _parser(p, state !== States.Binary);
      token = p.token;
